...
const Layout = ({ columnCount }) => {
  ...
  return (
    <div style={containerStyle}>
      {waferData.map((data) => (
        <div key={data.id} style={{ width: '100%', border: '1px solid #ddd', borderRadius: '8px', background: 'white', padding: '10px' }}>
          <h5 style={{ textAlign: 'center', margin: '0 0 10px 0' }}>Wafer #{data.id}</h5>
