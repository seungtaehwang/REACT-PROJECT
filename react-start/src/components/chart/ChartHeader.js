...
const Header = ({ onTypeChange, onDataSelect, onLibraryChange, onColumnCountChange }) => {
  return (
    <header style={{ padding: '15px 20px', background: '#316da8', color: 'white', display: 'flex', gap: '20px', alignItems: 'center' }}>
      <button onClick={() => onDataSelect('A 세트')} style={{ cursor: 'pointer' }}>데이터 A</button>
      <button onClick={() => onDataSelect('B 세트')} style={{ cursor: 'pointer' }}>데이터 B</button>
      ...
      <div style={{ marginLeft: 'auto', display: 'flex', gap: '15px' }}>
        ...
        <label>
          Chart Count: 
          <select onChange={(e) => onColumnCountChange(e.target.value)} style={{ marginLeft: '5px' }}>
