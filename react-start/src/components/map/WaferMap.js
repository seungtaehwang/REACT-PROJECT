...
const WaferMap = ({ WaferPixelsize, params }) => {
  ...
  const defectdatas = useMemo(() => {
    ...
    if (params.mapType === "DEFECT") return generateDefects(params, 150, 3, 2, 123); // (range, numRandom, numClusters, numScratches, seed)
    else return [];
  }, [params]);
  ...
  return (
    <>
      <canvas 
        ...
        style={{ background: '#fff', padding: `${CANVAS_PADDING}px`, boxSizing: 'border-box' }}
      />
      {hoveredDefect && (
        <div style={{
          position: 'fixed',
          ...
          Y: {hoveredDefect.y.toFixed(0)} nm<br/>
          TYPE: {hoveredDefect.type}<br/>
